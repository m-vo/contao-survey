<?php

declare(strict_types=1);

/*
 * @author  Moritz Vondano
 * @license MIT
 */

$GLOBALS['TL_LANG']['survey_question_type']['age'] = 'Alter';
$GLOBALS['TL_LANG']['survey_question_type']['matrix'] = 'Matrix';
$GLOBALS['TL_LANG']['survey_question_type']['order'] = 'Reihenfolge';
$GLOBALS['TL_LANG']['survey_question_type']['rating'] = 'Bewertung';
$GLOBALS['TL_LANG']['survey_question_type']['select'] = 'Auswahl';
$GLOBALS['TL_LANG']['survey_question_type']['text'] = 'Text';
$GLOBALS['TL_LANG']['survey_question_type']['longtext'] = 'Langtext';
