<?php

declare(strict_types=1);

/*
 * @author  Moritz Vondano
 * @license MIT
 */

$GLOBALS['TL_LANG']['tl_survey']['sections'] = ['Edit sections', 'Edit sections of survey ID %s'];
$GLOBALS['TL_LANG']['tl_survey']['csv_export'] = ['Export results', 'Export results of survey ID %s'];
$GLOBALS['TL_LANG']['tl_survey']['clearRecords'] = ['Clear all records', 'Clear all records of survey ID %s'];

$GLOBALS['TL_LANG']['tl_survey']['title'] = ['Title', 'Enter the title of this survey.'];
$GLOBALS['TL_LANG']['tl_survey']['frozen'] = ['Freeze survey', 'Lock editing of this survey. Only frozen surveys will be output in the frontend.'];
$GLOBALS['TL_LANG']['tl_survey']['details_legend'] = 'Submission details';
$GLOBALS['TL_LANG']['tl_survey']['note_submission'] = ['Submission note', 'Add a "thank you" message that will be shown after successful submission.'];
$GLOBALS['TL_LANG']['tl_survey']['button_label'] = ['Button label', 'Add an optional button.'];
$GLOBALS['TL_LANG']['tl_survey']['button_href'] = ['Button target'];

$GLOBALS['TL_LANG']['tl_survey']['frozen_disabled'] = 'The survey contains records and cannot be frozen.';
$GLOBALS['TL_LANG']['tl_survey']['frozen_survey'] = 'Frozen surveys cannot be edited to ensure consistent data records.';
